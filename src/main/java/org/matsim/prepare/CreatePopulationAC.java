 ...
 *
 */
public class CreatePopulationAC {

	private static final Logger log = Logger.getLogger(CreatePopulationAC.class);
	...
	public static void main(String[] args) throws IOException {
		...
		final String outputFilePopulation = "./scenarios/measures/input/serengeti-park-population-AC.xml.gz";

		Config config = ConfigUtils.createConfig();
		config.network().setInputFile(networkFile);
		Scenario scenario = ScenarioUtils.loadScenario(config);

		CreatePopulationAC popGenerator = new CreatePopulationAC(1000, 675, 1569, 543);
	...
	public CreatePopulationAC(int numberOfSafariVisitors, int safariParkplatzVisitors, int wasserlandParkplatzVisitors, int eickelohParkplatzVisitors) throws IOException {
	...
	public Scenario run(Scenario scenario) {
		...
		for (Id<Link> linkId : linkId2numberOfVisitorsSerengetiPark_EickelohParkplatzUsers.keySet()) {
			createSafariVisitors(scenario, rnd, linkId, linkId2numberOfVisitorsSerengetiPark_EickelohParkplatzUsers.get(linkId), this.min_freespeedTT, this.serengetiParkDestination, this.eickelohParkplatzDestination);
		}
