...
public class CreatePopulationABC {

	private static final Logger log = Logger.getLogger(CreatePopulationABC.class);
	...
	public static void main(String[] args) throws IOException {
		...
		final String networkFile = "./scenarios/measures/input/serengeti-park-network-AC.xml.gz";
		final String outputFilePopulation = "./scenarios/measures/input/serengeti-park-population-ABC.xml.gz";

		Config config = ConfigUtils.createConfig();
		config.network().setInputFile(networkFile);
		Scenario scenario = ScenarioUtils.loadScenario(config);

		CreatePopulationABC popGenerator = new CreatePopulationABC(1000, 675, 1569, 543, 7200);
	...
	public CreatePopulationABC(int numberOfSafariVisitors, int safariParkplatzVisitors, int wasserlandParkplatzVisitors, int eickelohParkplatzVisitors, int timeSlotDuration) throws IOException {
	...
	public Scenario run(Scenario scenario) {
		...
		for (int i = 0; i < numberOfTimeSlots; i++) {
			...
			for (Id<Link> linkId : partOfLinkId2numberOfVisitorsSerengetiPark_SerengetiParkplatzUsers.keySet()) {
				createSafariVisitors(scenario, rnd, linkId, partOfLinkId2numberOfVisitorsSerengetiPark_SerengetiParkplatzUsers.get(linkId), this.freespeedTravelTime, this.serengetiParkDestination, this.serengetiParkplatzDestination, i+1);
			}

			for (Id<Link> linkId : partOfLinkId2numberOfVisitorsSerengetiPark_WasserlandUsers.keySet()) {
				createSafariVisitors(scenario, rnd, linkId, partOfLinkId2numberOfVisitorsSerengetiPark_WasserlandUsers.get(linkId), this.freespeedTravelTime, this.serengetiParkDestination, this.wasserlandParkplatzDestination, i+1);
			}

			for (Id<Link> linkId : partOfLinkId2numberOfVisitorsSerengetiPark_EickelohParkplatzUsers.keySet()) {
				createSafariVisitors(scenario, rnd, linkId, partOfLinkId2numberOfVisitorsSerengetiPark_EickelohParkplatzUsers.get(linkId), this.freespeedTravelTime, this.serengetiParkDestination, this.eickelohParkplatzDestination, i+1);
			}
	...
	private void createVisitors(Scenario scenario, Random rnd, Id<Link> linkId, double odSum, String type, int timeSlot) {
		...
		for (int i = 0; i < odSum; i++) {
			...
			double startTime = calculateRandomlyDistributedValue( 10*3600. + this.timeSlotDuration * (timeSlot-1) , 1 * this.freespeedTravelTime); // randomly distributed
	...
	// ms: park opening at 10am
	private void createSafariVisitors(Scenario scenario, Random rnd, Id<Link> linkId, double odSum, double freespeedTT, String finalType, String parkingType, int timeSlot) {
		Population population = scenario.getPopulation();
		PopulationFactory popFactory = population.getFactory();
