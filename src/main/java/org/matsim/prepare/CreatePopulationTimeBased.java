...
public class CreatePopulationTimeBased {
...
//	private final ArrayList<Double> timeBasedDemand = new ArrayList<>( Arrays. asList(0.153808594, 0.304199219, 0.314453125, 0.141113281, 0.055175781, 0.024902344, 0.006347656, 0.) );
	...
	public static void main(String[] args) throws IOException {
		...
		final String outputFilePopulation = "./scenarios/input/population/serengeti-park-population-v1.0.xml.gz";
		...
		//CreatePopulationTimeBasedDemand popGenerator = new CreatePopulationTimeBasedDemand(2000, 1250, 1250, 0, twoLots, 1, 9.5*3600., 16.5*3600.);
	...
	public CreatePopulationTimeBased(int numberOfSafariVisitors, int safariParkplatzVisitors, int wasserlandParkplatzVisitors, ArrayList<String> availableParkingLots, int numberOfTimeSlots, double checkInOpeningTime, double checkInClosingTime) throws IOException {
...
//				double startTime = calculateNormallyDistributedTime(12 * 3600., 1* 3600.); // NORMALLY distributed
	...
	private void createSafariVisitors(Scenario scenario, Random rnd, Id<Link> linkId, double odSum, String finalType, String parkingType, int time_index) {
		...
		if (this.numberOfTimeSlots==1) { //Base Case , EickelohOpen
			...
			for (int i = 0; i < odSum; i++) {
				Person pers = popFactory.createPerson(Id.create("visitor_" + personCounter + "_" + linkId.toString() + "-" + finalType, Person.class));
...
//				double startTime = calculateRandomlyDistributedValue(12 * 3600., 2 * 3600.); // randomly distributed
			...
			for (int i = 0; i < odSum; i++) {
				Person pers = popFactory.createPerson(Id.create("visitor_" + personCounter + "_" + linkId.toString() + "_S" + (time_index+1) + "-" + finalType, Person.class));
